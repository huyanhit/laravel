	...
	public function getRecentNews($id = 0){
		...
		foreach ($result as $key => $val) {
            ...
            if(empty($result[$key]->image) || !file_exists('public/uploads/'.$this->images.'/'.$result[$key]->image)){
                ...
                $this->myFunction->cropImage(url('/').'/public/uploads/'.$this->images.'/'.$result[$key]->image,1,1,$this->thum_images,200);
                $result[$key]->image = url('/').'/public/uploads/'.$this->thum_images.'/'.$result[$key]->image;
            }
        }
        return $result;
	}
